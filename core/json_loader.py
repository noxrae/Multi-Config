...
def find_report_json(extracted_root: Path) -> Path:
    ...
    # Prefer Playwright default location first.
...
def list_test_json_files(extracted_root: Path) -> list[Path]:
    """Finds all JSON files that are likely test results (hex/random names), excluding report.json."""
    all_jsons = list(extracted_root.rglob("*.json"))
    # Filter out report.json and the normalized output if it happens to be there
    test_files = [
